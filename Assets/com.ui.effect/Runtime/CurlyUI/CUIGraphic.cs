...
namespace UnityEngine.UI.Effect
{
    ...
    public class CUIGraphic : BaseMeshEffect
    {
            ...
            for (int c = 2; c < 4; c++)
            {
                ...
                for (int p = 0; p < CUIBezierCurve.CubicBezierCurvePtNum; p++)
                {
                    if (curve.ControlPoints != null)
                    {
                        ...
                        {
                            controlPoints[p].x = rect.width - rect.width * rectTrans.pivot.x;
                        }

                        controlPoints[p].y = rect.height * p / (CUIBezierCurve.CubicBezierCurvePtNum - 1);
                        controlPoints[p].y -= rect.height * rectTrans.pivot.y;
                    }
                }
            }
        }
        ...
        {
            if (!IsActive() || UIGraphic == null)
                return;
            if (!(uiGraphic is SVGImage))
            {
            ...
            for (var index = 0; index < _verts.Count; index++)
            {
                ...
                var horRatio = (uiVertex.position.x + rect.width * pivot.x) / rect.width;
                var verRatio = (uiVertex.position.y + rect.height * pivot.y) / rect.height;
        ...
        public Vector2 GetBCurveSandwichSpacePoint(float _xTime, float _yTime)
        {
            var first = refCurves[0].GetPoint(_xTime*(refCurves[0].ControlPoints.Length-1)/ refCurves[0].ControlPoints.Length
            ) * (1 - _yTime) + refCurves[1].GetPoint(_xTime*(refCurves[1].ControlPoints.Length-1)/ refCurves[1].ControlPoints.Length
            ) * _yTime;
            var second = refCurves[2].GetPoint(_yTime*(refCurves[2].ControlPoints.Length-1)/ refCurves[2].ControlPoints.Length) * 
                (1 - _xTime) + refCurves[3].GetPoint(
                _yTime*(refCurves[3].ControlPoints.Length-1)/ refCurves[3].ControlPoints.Length) * _xTime;
            //return first;
