...
namespace UnityEngine.UI.Effect
{
    ...
    [AddComponentMenu("UI/Effects/UIGradient", 101)]
    public class UIGradient : BaseMeshEffect
    {
        private static readonly Vector2[] s_SplitedCharacterPosition = {Vector2.up, Vector2.one, Vector2.right, Vector2.zero};
        ...
        public override void ModifyMesh(VertexHelper vh, Graphic graphic)
        {
            ...
            for (var i = 0; i < vh.currentVertCount; i++)
            {
                ...
                {
                    color = Color.LerpUnclamped(Color.LerpUnclamped(m_Color1, m_Color2, normalizedPos.x), Color.LerpUnclamped(m_Color3, m_Color4, normalizedPos.x), normalizedPos.y);
                }
                else
                {
                    color = Color.LerpUnclamped(m_Color2, m_Color1, normalizedPos.y);
                }
                vertex.color *= (m_ColorSpace == ColorSpace.Gamma) ? color.gamma : (m_ColorSpace == ColorSpace.Linear) ? color.linear : color;
